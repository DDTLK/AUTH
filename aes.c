...
uint8_t get_sbox_inverse(uint8_t num)  {
  uint8_t ibox[256] = {
...
// This function produces 4(Nr+1) round keys. The round keys are used in 
// each round to decrypt the states.  
void key_expansion(uint8_t *expanded_key, uint8_t *key)  {
  int i,j;  
  uint8_t temp[4],k;

  int key_size = 128;//can be made into parameter; possible values = 128, 192, 256
  int Nk = key_size/32;
  int Nr = Nk+6;
  ...
  while (i < (4 * (Nr+1)))  {  
	...
	// This function that takes a four-byte input word and applies
	...
	temp[0] =  temp[0] ^ round_const[i/Nk];  
      }  
      else if (Nk > 6 && i % Nk == 4)  {
	// Function Subword()
	{
      ...
      expanded_key[i*4+0] = expanded_key[(i-Nk)*4+0] ^ temp[0];
...
void AddRoundKey(int round, uint8_t state[4][4], uint8_t *expanded_key)  {
  int i,j;
  for(i=0;i<4;i++)  {
    for(j=0;j<4;j++)  {
      state[j][i] ^= expanded_key[round * 4 * 4 + i * 4 + j];
    }
  }
}
...
// xtime is a macro: product of {02} and the argument to xtime modulo {1b} 
